...
## Pitfalls
- **Backward-compat shims** re-export from domain packages; import from the
  domain directly in new code, not these: `app/models.py`, `app/crud.py`,
  `app/api/deps.py`. (`app/models.py` also defines the shared `Message` schema.)
